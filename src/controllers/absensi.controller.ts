...
export const create = asyncHandler(async (req: any, res: Response) => {
  ...
  return successResponse(res, "Absensi berhasil dicatat", result, undefined, 201);
});
...
export const getBySantri = asyncHandler(async (req: any, res: Response) => {
  ...
  return successResponse(res, "History absensi santri berhasil diambil", result);
});
...
export const getByHalaqah = asyncHandler(async (req: any, res: Response) => {
  ...
  // JIKA REQUEST ADALAH REKAP BULANAN (?month=..&year=..)
  if (month && year) {
  ...
  // JIKA REQUEST ADALAH HARIAN (Default)
